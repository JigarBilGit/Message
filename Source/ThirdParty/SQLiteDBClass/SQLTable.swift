...
class SQLTable: NSObject, SQLTableProtocol {
    ...
    required override init() {
        ...
        guard (type(of: self).table) != nil else {
        ...
        let verified = SQLTable.verified[self.table]
        if verified == nil || !verified! {
            // Verify that the table exists in DB
            var sql = "SELECT name FROM sqlite_master WHERE type='table' AND lower(name)='\(self.table)'"
            let cnt = db.query(sql:sql).count
            if cnt == 1 {
                // Table exists, verify strutcure and then proceed
                verifyStructure()
                SQLTable.verified[self.table] = true
            } else if cnt == 0 {
                ...
                if rc == 0 {
                    assert(false, "Error creating table - \(self.table) with SQL: \(sql)")
                }
                SQLTable.verified[table] = true
            } else {
                assert(false, "Got more than one table in DB with same name! Count: \(cnt) for \(self.table)")
            }
    ...
    class func whereFor(type: FetchType) -> String {
        ...
        case .deleted:
            return " WHERE isRowDeleted"
            
        case .nondeleted:
            return " WHERE (NOT isRowDeleted OR isRowDeleted IS NULL)"
        }
    }
    ...
    class func remove(filter: String = "", force: Bool = false) -> Bool {
        ...
        var sql = "UPDATE \(table) SET isRowDeleted = ?, modified = ?"
        if force {
            params = nil
    ...
    class func clearTrash(filter: String = "") {
        let db = SQLiteDB.shared
        var sql = "DELETE FROM \(table) WHERE isRowDeleted"
        if !filter.isEmpty {
            // Use filter to delete
            sql += " AND \(filter)"
        }
    ...
    func delete(force: Bool = false) -> Bool {
        ...
        if let rid = data[key] {
            var params: [Any]? = [true, Date()]
            var sql = "UPDATE \(table) SET isRowDeleted = ?, modified = ? WHERE \(primaryKey())=\(rid)"
            if force {
                params = nil
