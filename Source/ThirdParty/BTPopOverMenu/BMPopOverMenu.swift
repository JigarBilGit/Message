...
extension BMPopOverMenu {
    ...
    public class func showForSender(sender: UIView, with menuArray: [BMMenuObject], menuImageArray: [Imageable]? = nil, popOverPosition: BMPopOverPosition = .automatic, config: BMConfiguration? = nil, done: ((NSInteger)->())?, cancel: (()->())? = nil) {
        BMPopOverMenu.shared.showForSender(sender: sender, or: nil, with: menuArray, menuImageArray: menuImageArray, popOverPosition: popOverPosition, config: config, done: done, cancel: cancel)
    }

    public class func showForEvent(event: UIEvent, with menuArray: [BMMenuObject], menuImageArray: [Imageable]? = nil, popOverPosition: BMPopOverPosition = .automatic, config: BMConfiguration? = nil, done: ((NSInteger)->())?, cancel: (()->())? = nil) {
        BMPopOverMenu.shared.showForSender(sender: event.allTouches?.first?.view!, or: nil, with: menuArray, menuImageArray: menuImageArray, popOverPosition: popOverPosition, config: config, done: done, cancel: cancel)
    }

    public class func showFromSenderFrame(senderFrame: CGRect, with menuArray: [BMMenuObject], menuImageArray: [Imageable]? = nil,popOverPosition: BMPopOverPosition = .automatic, config: BMConfiguration? = nil, done: ((NSInteger)->())?, cancel: (()->())? = nil) {
        BMPopOverMenu.shared.showForSender(sender: nil, or: senderFrame, with: menuArray, menuImageArray: menuImageArray, popOverPosition: popOverPosition, config: config, done: done, cancel: cancel)
    }
    
}

fileprivate enum BMPopOverMenuArrowDirection {
    case up
    case down
}

public enum BMPopOverPosition {
    case automatic
    case alwaysAboveSender
    case alwaysUnderSender
}

public class BMPopOverMenu: NSObject, BMPopOverMenuViewDelegate {
    
    var sender: UIView?
    var senderFrame: CGRect?
    var menuNameArray: [BMMenuObject]!
    var menuImageArray: [Imageable]!
    var done: ((Int)->())?
    var cancel: (()->())?
    var configuration = BMConfiguration()
    var popOverPosition: BMPopOverPosition = .automatic
    ...
    fileprivate lazy var popOverMenuView: BMPopOverMenuView = {
        let menu = Communication.BMPopOverMenuView(frame: CGRect.zero)
        menu.alpha = 0
    ...
    class var shared: BMPopOverMenu {
        struct Manager {
            static let instance = BMPopOverMenu()
        }
        return Manager.instance
    }
    
    public func showForSender(sender: UIView?, or senderFrame: CGRect?, with menuNameArray: [BMMenuObject]!, menuImageArray: [Imageable]? = nil, popOverPosition: BMPopOverPosition = .automatic, config: BMConfiguration? = nil, done: ((Int)->())?, cancel: (()->())? = nil) {
        ...
        self.configuration = config ?? BMConfiguration()
        self.done = done
        self.cancel = cancel
    ...
    fileprivate func adjustPostionForPopOverMenu() {
        self.backgroundView.frame = CGRect(x: 0, y: 0, width: UIScreen.bi_width(), height: UIScreen.bi_height())
    ...
    fileprivate var arrowDirection: BMPopOverMenuArrowDirection = .up
    fileprivate var popMenuHeight: CGFloat {
        return configuration.menuRowHeight * CGFloat(self.menuNameArray.count) + BM.DefaultMenuArrowHeight
    }
    ...
    fileprivate func configureSenderRect() {
        ...
        senderRect.origin.y = min(UIScreen.bi_height(), senderRect.origin.y)
        ...
        if popOverPosition == .alwaysAboveSender {
            ...
            if senderRect.origin.y + senderRect.size.height/2 < UIScreen.bi_height()/2 {
                arrowDirection = .up
            } else {
                arrowDirection = .down
            }
    ...
    fileprivate func configurePopMenuOriginX() {
        ...
        let menuCenterX: CGFloat = configuration.menuWidth/2 + BM.DefaultMargin
        var menuX: CGFloat = 0
        if senderXCenter.x + menuCenterX > UIScreen.bi_width() {
            senderXCenter.x = min(senderXCenter.x - (UIScreen.bi_width() - configuration.menuWidth - BM.DefaultMargin), configuration.menuWidth - BM.DefaultMenuArrowWidth - BM.DefaultMargin)
            menuX = UIScreen.bi_width() - configuration.menuWidth - BM.DefaultMargin
        } else if senderXCenter.x - menuCenterX < 0 {
            senderXCenter.x = max(BM.DefaultMenuCornerRadius + BM.DefaultMenuArrowWidth, senderXCenter.x - BM.DefaultMargin)
            menuX = BM.DefaultMargin
        } else {
    ...
    fileprivate func configurePopMenuFrame() {
        ...
        if arrowDirection == .up {
            ...
            if (popMenuFrame.origin.y + popMenuFrame.size.height > UIScreen.bi_height() - safeAreaInset.bottom) {
                popMenuFrame = CGRect(x: popMenuOriginX, y: (senderRect.origin.y + senderRect.size.height), width: configuration.menuWidth, height: UIScreen.bi_height() - popMenuFrame.origin.y - BM.DefaultMargin - safeAreaInset.bottom)
            }
        } else {
            ...
            if popMenuFrame.origin.y  < safeAreaInset.top {
                popMenuFrame = CGRect(x: popMenuOriginX, y: BM.DefaultMargin + safeAreaInset.top, width: configuration.menuWidth, height: senderRect.origin.y - BM.DefaultMargin - safeAreaInset.top)
            }
        }
    }
    ...
    fileprivate func configureMenuArrowPoint() {
        ...
        if point.x + menuCenterX > UIScreen.bi_width() {
            point.x = min(point.x - (UIScreen.bi_width() - configuration.menuWidth - BM.DefaultMargin), configuration.menuWidth - BM.DefaultMenuArrowWidth - BM.DefaultMargin)
        } else if point.x - menuCenterX < 0 {
            point.x = max(BM.DefaultMenuCornerRadius + BM.DefaultMenuArrowWidth, point.x - BM.DefaultMargin)
        } else {
    ...
    fileprivate func showIfNeeded() {
        if self.isOnScreen == false {
            ...
            UIView.animate(withDuration: BM.DefaultAnimationDuration, animations: {
    ...
    fileprivate func doneActionWithSelectedIndex(selectedIndex: NSInteger) {
        ...
        UIView.animate(withDuration: BM.DefaultAnimationDuration,
                       animations: {
    ...
    // MARK: - BMPopOverMenuViewDelegate -
    
    func BMPopOverMenuView(didSelect index: Int) {
...
fileprivate protocol BMPopOverMenuViewDelegate: NSObjectProtocol {
    
    func BMPopOverMenuView(didSelect index: Int)
    
}

fileprivate class BMPopOverMenuView: UIControl {
    
    fileprivate var menuNameArray: [BMMenuObject]!
    fileprivate var menuImageArray: [Imageable]?
    fileprivate var arrowDirection: BMPopOverMenuArrowDirection = .up
    fileprivate weak var delegate: BMPopOverMenuViewDelegate?
    fileprivate var configuration = BMConfiguration()
    ...
    fileprivate func showWithAnglePoint(point: CGPoint, frame: CGRect, menuNameArray: [BMMenuObject]!, menuImageArray: [Imageable]?, config: BMConfiguration? = nil, arrowDirection: BMPopOverMenuArrowDirection, delegate: BMPopOverMenuViewDelegate?) {
    ...
    fileprivate func repositionMenuTableView() {
        var menuRect: CGRect = CGRect(x: 0, y: BM.DefaultMenuArrowHeight, width: frame.size.width, height: frame.size.height - BM.DefaultMenuArrowHeight)
        if (arrowDirection == .down) {
            menuRect = CGRect(x: 0, y: 0, width: frame.size.width, height: frame.size.height - BM.DefaultMenuArrowHeight)
        }
    ...
    func getBackgroundPath(arrowPoint: CGPoint) -> UIBezierPath {
        ...
        if (arrowDirection == .up){
            path.move(to: CGPoint(x: arrowPoint.x - BM.DefaultMenuArrowWidth, y: BM.DefaultMenuArrowHeight))
            path.addLine(to: CGPoint(x: arrowPoint.x, y: 0))
            path.addLine(to: CGPoint(x: arrowPoint.x + BM.DefaultMenuArrowWidth, y: BM.DefaultMenuArrowHeight))
            path.addLine(to: CGPoint(x:viewWidth - radius, y: BM.DefaultMenuArrowHeight))
            path.addArc(withCenter: CGPoint(x: viewWidth - radius, y: BM.DefaultMenuArrowHeight + radius),
                        radius: radius,
                        startAngle: .pi / 2 * 3,
                        endAngle: 0,
                        clockwise: true)
            ...
            path.addArc(withCenter: CGPoint(x: radius, y: viewHeight - radius),
                        ...
                        endAngle: .pi,
                        clockwise: true)
            path.addLine(to: CGPoint(x: 0, y: BM.DefaultMenuArrowHeight + radius))
            path.addArc(withCenter: CGPoint(x: radius, y: BM.DefaultMenuArrowHeight + radius),
                        radius: radius,
                        startAngle: .pi,
                        endAngle: .pi / 2 * 3,
                        clockwise: true)
            ...
            path.move(to: CGPoint(x: arrowPoint.x - BM.DefaultMenuArrowWidth, y: viewHeight - BM.DefaultMenuArrowHeight))
            path.addLine(to: CGPoint(x: arrowPoint.x, y: viewHeight))
            path.addLine(to: CGPoint(x: arrowPoint.x + BM.DefaultMenuArrowWidth, y: viewHeight - BM.DefaultMenuArrowHeight))
            path.addLine(to: CGPoint(x: viewWidth - radius, y: viewHeight - BM.DefaultMenuArrowHeight))
            path.addArc(withCenter: CGPoint(x: viewWidth - radius, y: viewHeight - BM.DefaultMenuArrowHeight - radius),
                        radius: radius,
                        startAngle: .pi / 2,
                        endAngle: 0,
                        clockwise: false)
            ...
            path.addArc(withCenter: CGPoint(x: radius, y: radius),
                        ...
                        endAngle: .pi,
                        clockwise: false)
            path.addLine(to: CGPoint(x: 0, y: viewHeight - BM.DefaultMenuArrowHeight - radius))
            path.addArc(withCenter: CGPoint(x: radius, y: viewHeight - BM.DefaultMenuArrowHeight - radius),
                        radius: radius,
                        startAngle: .pi,
                        endAngle: .pi / 2,
                        clockwise: false)
...
extension BMPopOverMenuView: UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        self.delegate?.BMPopOverMenuView(didSelect: indexPath.row)
    }
...
extension BMPopOverMenuView: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell: BMPopOverMenuCell = BMPopOverMenuCell(style: .default, reuseIdentifier: BM.PopOverMenuTableViewCellIndentifier)
